...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	while (1)
	{
		...
		DrawFormatString(0, 0, GetColor(255, 255, 255), "進撃面白い");

		for (int x = 0; x < 20; x++) {
			DrawLine(30 * x + ShakeYoko, -400 + ShakeTate, 30 * x + ShakeYoko, 800 + ShakeTate, GetColor(0, 0, 0), TRUE);
		}

		for (int y = 0; y < 20; y++) {
			DrawLine(-600 + ShakeYoko, y * 20 + ShakeTate, 1200 + ShakeYoko, y * 20 + ShakeTate, GetColor(0, 0, 0), TRUE);
		}
		//---------  ここまでにプログラムを記述  ---------//
